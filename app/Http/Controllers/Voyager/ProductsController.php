...
class ProductsController extends VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $downloadsAssociated = $product->download()->get();
        $activeDownload = $product->download()->where('active',1)->get();
   

        $categoriesForProduct = $product->categories()->get();
        $platformsForProduct = $product->platforms()->get();
     
        $sectionsForProduct = $product->sections()->get();
      
        return Voyager::view($view, compact('dataType', 'dataTypeContent',
         'isModelTranslatable','allCategories','allPlatforms','allDownloads',
         'categoriesForProduct','platformsForProduct',
         'allSections','sectionsForProduct',
         'downloadsAssociated','activeDownload'
        ));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        SectionProduct::where('product_id',$id)->delete();
        if($request->section){
       ...
       DownloadProduct::where('product_id',$id)->update(['active'=> 0]);
       
        if($request->download){
            
            foreach($request->download as $download){
              DownloadProduct::where('product_id',$id)->where('download_id',$download)->delete();
    ...
    public function create(Request $request)
    {
        ...
        $allPlatforms = Platform::all();
        $platformsForProduct = collect([]);

        $allSections = Section::all();
        $sectionsForProduct = collect([]);
        
        $downloadsAssociated = collect([]);
        
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable',
        'allCategories','categoriesForProduct','allPlatforms','platformsForProduct',
    'allSections','sectionsForProduct','downloadsAssociated'));
    }
